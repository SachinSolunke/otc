#!/usr/bin/env python3
# OTC MATH FORMULAS ANALYZER v1.2 - Sliding Window & New Formulas
...
import json # Not actively used for saving/loading stats in this version's main flow for simplicity
...
BACKTEST_WINDOW_DAYS = 30 # Analyze formulas based on the last N days of data
MIN_HIT_RATE_SUGGESTION = 0.39 # (40%+) within the window
MIN_TRIES_SUGGESTION_CONFIG = 10 # Desired min tries, will be adjusted if window is smaller
...
def parse_data_line(line_str):
    try:
        ...
        if not (re.fullmatch(r"\d{3}",p1_str) and 
                re.fullmatch(r"\d{2}",jodi_str) and 
                re.fullmatch(r"\d{3}",p2_str)): return None
        dt = datetime.strptime(date_part_str, "%d-%m-%Y")
        
        p1_int = [int(d) for d in p1_str]
        j_int = [int(d) for d in jodi_str]
        p2_int = [int(d) for d in p2_str]
        all_digits = p1_int + j_int + p2_int
        ...
        return {
            ...
            "p1_digits_int": p1_int, "p2_digits_int": p2_int,
            "all_8_digits_int": all_digits }
    except Exception: # Catch any parsing error for the line
        return None
...
def f_prev_oc_anks(prev_data, params=None):
    return {prev_data['open_ank'], prev_data['close_ank']} if prev_data else set()
def f_prev_jodi_digits(prev_data, params=None):
    return {prev_data['jodi_d1'], prev_data['jodi_d2']} if prev_data else set()
...
def f_ank_weighted_oc_plus_x(prev_data, params): # NEW
    if not prev_data or "wO" not in params or "wC" not in params or "X" not in params: return set()
    try:
        oa,ca=int(prev_data['open_ank']),int(prev_data['close_ank']); wO,wC,x = params["wO"],params["wC"],params["X"]
        return {str( (oa*wO + ca*wC + x) % 10 )}
    except: return set()
...
def f_ank_panel_all_digits_sum_plus_x(prev_data, params): # NEW
    if not prev_data or "panel" not in params or "X" not in params: return set()
    try: panel_sum = sum(prev_data[params["panel"]+"_digits_int"]); x = params["X"]; return {str((panel_sum % 10 + x) % 10)}
    except: return set()
def f_ank_panel_all_digits_prod_plus_x(prev_data, params): # NEW
    if not prev_data or "panel" not in params or "X" not in params: return set()
    try:
        prod = 1; x = params["X"]
        for digit in prev_data[params["panel"]+"_digits_int"]: prod *= digit if digit != 0 else 1 # Treat 0 as 1 for product
        return {str((prod % 10 + x) % 10)}
    except: return set()
...
# P1 digit and P2 digit based - NEW
def f_ank_p1d_op_p2d_plus_x(prev_data, params):
    if not prev_data or "p1_idx" not in params or "p2_idx" not in params or "op" not in params or "X" not in params: return set()
    try:
        p1_d = prev_data["p1_digits_int"][params["p1_idx"]]
        p2_d = prev_data["p2_digits_int"][params["p2_idx"]]
        x = params["X"]; res = 0
        if params["op"] == "add": res = p1_d + p2_d
        elif params["op"] == "sub": res = abs(p1_d - p2_d)
        return {str((res % 10 + x) % 10)}
    except: return set()
...
for x_val in range(10):
    ...
    ALL_FORMULA_SPECS[f"TotalSumAll_X{x_val}"] = {"func": f_ank_total_sum_all_plus_x, "params": {"X": x_val}, "display": f"TotalAll8+({x_val})", "type": "ank"}
for p_type_key in ["p1", "p2"]:
    for x_val in range(5): # Reduced X for panel sums
        ALL_FORMULA_SPECS[f"PanelAllSum_{p_type_key.upper()}_X{x_val}"] = {"func":f_ank_panel_all_digits_sum_plus_x, "params":{"panel":p_type_key, "X":x_val}, "display":f"Sum{p_type_key.upper()}+{x_val}", "type":"ank"}
        ALL_FORMULA_SPECS[f"PanelAllProd_{p_type_key.upper()}_X{x_val}"] = {"func":f_ank_panel_all_digits_prod_plus_x, "params":{"panel":p_type_key, "X":x_val}, "display":f"Prod{p_type_key.upper()}+{x_val}", "type":"ank"}
...
for ank_t_key in ["open_ank", "close_ank", "jodi_d1", "jodi_d2"]:
    ...
    if ank_t_key in ["open_ank", "close_ank"]:
        ...
        for w1 in [1,2]: # For weighted OC anks
            for w2 in [1,2]:
                 if w1==w2 and w1==2: continue # Avoid 2,2 if 1,1 and 1,2/2,1 exist
                 ALL_FORMULA_SPECS[f"WeightedOC_wO{w1}wC{w2}_X0"] = {"func":f_ank_weighted_oc_plus_x, "params":{"wO":w1,"wC":w2,"X":0}, "display":f"wO{w1}+wC{w2}", "type":"ank"}


panel_indices = [(0,1), (0,2), (1,2)] # For specific digit ops
panel_extremes_ops = {"add":"+", "sub":"-"}
for p_type_key in ["p1", "p2"]:
    for op_key_str, op_sym_char in panel_extremes_ops.items(): # Use extremes ops
    ...
    for p1_idx_val in range(3): # For P1[i] op P2[j]
        for p2_idx_val in range(3):
            for op_key_str, op_sym_char in panel_extremes_ops.items(): # add/sub
                 ALL_FORMULA_SPECS[f"P1D{p1_idx_val}{op_sym_char}P2D{p2_idx_val}_X0"] = {"func":f_ank_p1d_op_p2d_plus_x, "params":{"p1_idx":p1_idx_val, "p2_idx":p2_idx_val, "op":op_key_str, "X":0}, "display":f"P1[{p1_idx_val}]{op_sym_char}P2[{p2_idx_val}]", "type":"ank"}
# --- END FORMULA REGISTRATION ---

def backtest_all_formulas(full_historical_data):
    data_for_backtest = []
    current_backtest_window = BACKTEST_WINDOW_DAYS

    if len(full_historical_data) > current_backtest_window :
        data_for_backtest = full_historical_data[-(current_backtest_window + 1):]
        print(C_INFO_BRIGHT + f"Backtesting {len(ALL_FORMULA_SPECS)} OTC Ank formulas on last {len(data_for_backtest)-1} of {len(full_historical_data)} days (Sliding Window: {current_backtest_window})...")
    else:
        data_for_backtest = full_historical_data
        current_backtest_window = max(0, len(data_for_backtest)-1) # Actual window size
        print(C_INFO_BRIGHT + f"Backtesting {len(ALL_FORMULA_SPECS)} OTC Ank formulas on all {current_backtest_window} available days (Data < Window)...")
    ...
    if len(data_for_backtest) < 2: print(C_WARNING+"Not enough data in window for backtest."); return stats, 0

    actual_tries_in_window = len(data_for_backtest) - 1
    ...
    return stats, actual_tries_in_window

def get_otc_suggestions_for_tomorrow(latest_day_data, all_formula_stats, current_min_tries_for_filter):
    suggestions = [] 
    ...
    for f_id, perf_data, hit_rate in eligible_formulas:
        ...
        if generated_values: # Only add if formula produces anks
            suggestions.append({
...
def display_performance_summary(all_stats, effective_window_days):
    # ... (title updated, same as v1.2.1 otherwise)
    print_box_top(c=C_INFO_BRIGHT); print_box_line(f"{C_ACCENT_BRIGHT}Math Formula Performance (OTC Anks - Last {effective_window_days} Days)", bc=C_INFO_BRIGHT, align="center")
    ...
    for f_id, data in sorted_stats[:20]: 
        ...
        clr = C_SUCCESS if rate >= MIN_HIT_RATE_SUGGESTION*100 and data['tries'] >= MIN_TRIES_SUGGESTION_CONFIG else C_PRIMARY # Use config for color
...
def display_final_otc_suggestions(market_name, suggestions_from_formulas, effective_window_days): # Added window days for context
    # ... (same as v1.2.1, added window days to title)
    tomorrow = (datetime.now() + timedelta(days=1)).strftime('%d-%m-%Y (%A)')
    print_box_top(c=C_SUCCESS_BRIGHT)
    print_box_line(f"{C_ACCENT_BRIGHT}OTC Ank Suggestions for {market_name} - {tomorrow} (Based on last {effective_window_days} days)", bc=C_SUCCESS_BRIGHT, align="center")
    ...
    if not suggestions_from_formulas:
        ...
        else: print_box_line(C_WARNING+f"Could not determine {NUM_ANK_SUGGESTIONS_COMBINED} anks.", bc=C_SUCCESS_BRIGHT, align="center", p=1)
    print_box_bottom(c=C_SUCCESS_BRIGHT)

def main():
    show_banner()
    # Market Selection ... (same as v1.2.1)
    ...
    all_formula_stats, actual_tries_in_window = backtest_all_formulas(full_historical_data)
    
    effective_min_tries = min(MIN_TRIES_SUGGESTION_CONFIG, actual_tries_in_window)
    if actual_tries_in_window < MIN_TRIES_SUGGESTION_CONFIG and actual_tries_in_window > 0 :
        print(C_WARNING_BRIGHT + f"Note: MIN_TRIES for suggestion adjusted to {effective_min_tries} (max possible tries in window: {actual_tries_in_window}). Configured MIN_TRIES is {MIN_TRIES_SUGGESTION_CONFIG}.")
    elif actual_tries_in_window == 0 :
         print(C_ERROR_BRIGHT + f"Not enough data in window ({len(full_historical_data)} days, window {BACKTEST_WINDOW_DAYS}) for any backtest tries."); sys.exit(1)


    if all_formula_stats: display_performance_summary(all_formula_stats, actual_tries_in_window) # Pass actual window days
    ...
    otc_ank_suggestions = get_otc_suggestions_for_tomorrow(latest_day_data, all_formula_stats, effective_min_tries)


    if otc_ank_suggestions:
        display_final_otc_suggestions(market_name, otc_ank_suggestions, actual_tries_in_window) # Pass actual window days
    else: 
        window_display = BACKTEST_WINDOW_DAYS if len(full_historical_data) > BACKTEST_WINDOW_DAYS else max(0, len(full_historical_data)-1)
        print(C_WARNING_BRIGHT + f"\nNo Math Formulas for {market_name} met the required hit rate ({MIN_HIT_RATE_SUGGESTION*100:.0f}%) and min tries ({effective_min_tries}) within the last {window_display} days for an OTC Ank suggestion today.")
...
if __name__ == "__main__":
    ...
    finally: print(C_PRIMARY_BRIGHT+f"\n✨ Thank you for using {APP_NAME}! Keep trying, keep improving! ✨"+C_RESET)
